...
import (
	...
	"github.com/ascenmmo/tcp-server/clients/suppliers/cb"
	"github.com/ascenmmo/tcp-server/clients/suppliers/hasher"
...
type ClientJsonRPC struct {
	...
	fallbackGameConnections fallbackGameConnections
...
func New(endpoint string, opts ...Option) (cli *ClientJsonRPC) {
	...
	cli = &ClientJsonRPC{
		...
		name:         hostname + "_" + "github.com/ascenmmo/tcp-server",
	}
	cli.applyOpts(opts)
	cli.rpc = jsonrpc.NewClient(endpoint, cli.rpcOpts...)
	cli.cb = cb.NewCircuitBreaker("github.com/ascenmmo/tcp-server", cli.cbCfg)
	return
}

func (cli *ClientJsonRPC) GameConnections() *ClientGameConnections {
	return &ClientGameConnections{ClientJsonRPC: cli}
}
