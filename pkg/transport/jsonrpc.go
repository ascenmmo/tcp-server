...
func (srv *Server) doSingleBatch(ctx *fiber.Ctx, request baseJsonRPC) (response *baseJsonRPC) {
	...
	switch method {
	case "gameconnections.setsendmessage":
		return srv.httpGameConnections.setSendMessage(ctx, request)
	case "gameconnections.getmessage":
		return srv.httpGameConnections.getMessage(ctx, request)
	case "gameconnections.removeuser":
		return srv.httpGameConnections.removeUser(ctx, request)
	case "serversettings.getconnectionsnum":
		return srv.httpServerSettings.getConnectionsNum(ctx, request)
	case "serversettings.healthcheck":
		return srv.httpServerSettings.healthCheck(ctx, request)
	case "serversettings.getserversettings":
